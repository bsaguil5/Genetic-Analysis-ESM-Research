...
def load_data_from_csv(file_path, text_size=0.3, random_state=13):
    ...
    # AUTOMATIC FORMAT DETECTION - handles both original and expanded datasets
    if 'sequence' in df.columns:
        # NEW EXPANDED FORMAT (chat_labeled_sequences_expanded.csv)
        # Only use confident labels (not needing manual review)
        confident_df = df[df['needs_manual_review'] == False].copy()
        print(f"Loaded {len(df):,} total sequences, using {len(confident_df):,} confident labels")
        
        data = confident_df['sequence'].values
        labels = confident_df['label'].values
    else:
        # ORIGINAL FORMAT (training_labels.csv) 
        # Column names have spaces: ' Sequence', ' Label'
        data = df[' Sequence'].values
        labels = df[' Label'].values
    
    # Remove any NaN values
    valid_mask = pd.notna(data) & pd.notna(labels)
    data = data[valid_mask]
    labels = labels[valid_mask]
    
    print(f"Final dataset: {len(data):,} sequences with labels")
